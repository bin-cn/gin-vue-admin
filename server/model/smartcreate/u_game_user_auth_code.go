...
type GameUserAuthCode struct {
	...
	RoleGameId      *string `json:"roleGameId" form:"roleGameId" gorm:"uniqueIndex;comment:游戏角色ID;column:role_game_id;"`                  //游戏角色ID
	...
	UserId          *int    `json:"userId" form:"userId" gorm:"comment:所输用户ID;column:user_id;" binding:"required"`                        //用户ID

	// 在结构体中新增如下字段
	MainServerZone    *string `json:"mainServerZone" form:"mainServerZone" gorm:"comment:主区服;column:main_server_zone;"`               //主区服
	AccountInternalId *string `json:"accountInternalId" form:"accountInternalId" gorm:"comment:游戏账户内部ID;column:account_internal_id;"` //游戏账户内部ID
